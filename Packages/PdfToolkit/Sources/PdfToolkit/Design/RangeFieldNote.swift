...
/// counterpart to Merge's per-row notes. An invalid range is a genuine error (in Split it disables
/// Run), so it reads red here; the softer amber `fieldWarning` is reserved for non-error advisories
/// (e.g. passwords that don't match yet), so severity is legible from the color, not just the icon.
struct RangeFieldNote: View {
    let text: String
    var systemImage: String
    /// Red error + triangle when true (a bad range); accent-tinted when false (a valid summary).
    ...
    var body: some View {
        // The bad-range state is signaled by red vs the accent; when the user has asked not to rely
