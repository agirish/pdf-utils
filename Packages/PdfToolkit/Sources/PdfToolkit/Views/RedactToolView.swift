...
struct RedactToolView: View {
    ...
    private func reloadDocumentForSelection() async {
        ...
        do {
            ...
            // app has to be force-quit. Redact shows the annotations so the user can mark over them, so
            // it can't strip them the way Crop's marquee does.
            result.1?.log(tool: Tool.redact.title, url: url, stripped: false)
